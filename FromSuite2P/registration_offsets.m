function [dv, corr, regdata] = registration_offsets(data, ops, removeMean,varargin)
% updated 20170712: to use RefImage, use 4th argument. 
%% Parameters
...
for bi = 1:nBatches
  ...
  ConvSTD = getOr(ops, {'PhaseCorrBlurSTD'}, 1); % by default, 1.
  ...
  [dmax, ix] = max(dmax, [], 2);
%   [dmax, iy] = max(corrClipSmooth(:), [], 1);
%   [iy,ix]=ind2sub(size(corrClipSmooth),iy);
