function [ops, stat, res] = fastClustNeuropilCoef_kh(ops, U, Sv)
% 
...
if ops.ShowCellMap
    figh=myfigure('CellMap');
    set(figh, 'Units', 'pixels', 'position', [100 100 900 900])
    colormap('hsv')
...
Nk = ops.Nk0;
Nkiter = round(linspace(ops.Nk0, ops.Nk, niter-2)); % sequence of reducing clusters
Nkiter(end+1:(niter+1)) = ops.Nk; % make the last sequence longer. 
...
% Finv = (Fs*Fs')\Fs;
do_check_neuropil_contribuition = 0;
...
for k = 1:niter
    ...
    PixL = PixL';
    %%
    if do_check_neuropil_contribuition
        ii=10; 
        subplot(2,2,1); imagesc(reshape(neuropil(ii,:),Ly,Lx)); title('neuripil')
        subplot(2,2,2); imagesc(reshape(Ucell(ii,:),Ly,Lx));  title('Ucell')
        subplot(2,2,3); imagesc(reshape(Uneu(ii,:),Ly,Lx)); title('original U')
    end
    %%
...
%         V = max(0, min(10 * reshape(lam, Ly, Lx), 1));
        ...
        axis off
        drawnow
        fprintf('\nresidual variance is %2.6f time %2.2f \n', err(k), toc)
    end
...
newindx = cumsum(ison); 
if newindx(1)==0 % in case first cluster is merged into another cluster.
    newindx = 1+newindx;
end
...
save(sprintf('%s/F_%s_%s_plane%d_Ch%d_Nk%d.mat', ops.ResultsSavePath, ...
    ops.mouse_name, ops.date, ops.PlaneID,ops.ChannelID, Nk),  'ops', 'res', 'stat')
