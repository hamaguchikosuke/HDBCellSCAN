...
if nchannels ==1
    Ref_channel         = getOr(ops, {'Ref_channel'}, 1);   
else
   error('Reference channel is not defined in db');
end
...
%                     nchannels*nplanes*nfmax]; nchannels];
%             end
            ichanset = [offset + nchannels*(iplane0-1) + [Ref_channel;...
                nchannels*nplanes*nfmax]; nchannels];
    ...
    for j = 1:length(fs{k})
        ...
        for ss=1:nSteps
            ...
            if red_align
                ...
                %             ichanset = StartFrames-1+[rchannel; nChunk; nchannels];
                scan_index = (StartFrames(ss)+rchannel-1):nchannels:EndFrames(ss);
            else
                scan_index = (StartFrames(ss)+ichannel-1):nchannels:EndFrames(ss);
            end
        %         data = loadFramesBuff2(fs{k}(j).name, ichanset(1), ichanset(2), ichanset(3), [],[],ops.temp_tiff);
        ...
        if ops.doRegistration
            ...
            if red_align
                ...
                scan_index = (StartFrames(ss)-1):nchannels:EndFrames(ss);           
                data = BigTiffReader(ops.temp_tiff, scan_index);
...
for i = 1:numPlanes    
    ...
    if ~isempty(ops.RegFileTiffLocation)
        ...
        write_reg_tave_to_tiff(fid{i}, ops1{i}, i,ops.RegFile_xtime);
    end    
