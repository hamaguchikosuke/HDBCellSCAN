function varargout = CaRoiGu_v02(varargin)
% CAROIGU_V02 MATLAB code for CaRoiGu_v02.fig
%      CAROIGU_V02, by itself, creates a new CAROIGU_V02 or raises the existing
%      singleton*.
%
%      H = CAROIGU_V02 returns the handle to a new CAROIGU_V02 or the handle to
%      the existing singleton*.
%
%      CAROIGU_V02('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CAROIGU_V02.M with the given input arguments.
%
%      CAROIGU_V02('Property','Value',...) creates a new CAROIGU_V02 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CaRoiGu_v02_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CaRoiGu_v02_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CaRoiGu_v02

% Last Modified by GUIDE v2.5 08-May-2018 17:09:06

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @CaRoiGu_v02_OpeningFcn, ...
                   'gui_OutputFcn',  @CaRoiGu_v02_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CaRoiGu_v02 is made visible.
function CaRoiGu_v02_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CaRoiGu_v02 (see VARARGIN)

% Choose default command line output for CaRoiGu_v02
...
% UIWAIT makes CaRoiGu_v02 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CaRoiGu_v02_OutputFcn(hObject, eventdata, handles) 
...
if  get(handles.rb_FinalizeSignal,'Value') && ...
  ...
  for pp=1:length(PlaneChString)
      MinClustSize = handles.DB.clustrules.MinClust;
      ProcFile = sprintf('%s/Fsig_%s_%s_%s_MinClust%d_proc.mat', ops0.ResultsSavePath, ...
          ops0.mouse_name, ops0.date, PlaneChString{pp},MinClustSize);
      ProcSpkFile = sprintf('%s/Fsig_%s_%s_%s_MinClust%d_procSpk.mat', ops0.ResultsSavePath, ...
          ops0.mouse_name, ops0.date, PlaneChString{pp},MinClustSize);
