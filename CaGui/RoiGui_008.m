function varargout = RoiGui_008(varargin)
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @RoiGui_008_OpeningFcn, ...
                   'gui_OutputFcn',  @RoiGui_008_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%%
% --- Executes just before RoiGui_008 is made visible.
function RoiGui_008_OpeningFcn(hObject, eventdata, h, varargin)
% This function has no output args, see OutputFcn.
% varargin   command line arguments to RoiGui_008 (see VARARGIN)

h=init_all(hObject,h);
...
% UIWAIT makes RoiGui_008 wait for user response (see UIRESUME)
% uiwait(h.figure1);

function h=init_all(hObject,h)

h=init_icons(hObject,h);
h=init_movie_timer(hObject,h);
h.control_on = 0;
h.shift_on = 0;
try delete(h.axes_left.Children); end % clean up figures 
 
h.output = hObject;

function h=init_icons(hObject,h)
...
function h=init_movie_timer(hObject,h)
h.movie_timer = timer ;
...
function varargout = RoiGui_008_OutputFcn(hObject, eventdata, h) 
...
%     end
    ...
    if any(size(h.dat.res.iclust1)==1) % vector to matrix
        h.dat.res.iclust1=reshape(h.dat.res.iclust1,h.dat.res.Ly,h.dat.res.Lx);
    end
...
if isfield(h.dat.cl,'shift_labelID')
    % OK
else
    N=length(h.dat.cl.redcell);
    h.dat.cl.shift_labelID = getOr(h.dat.cl,'shift_labelID',zeros(N, 1));
end
...
h.dat.graph.crange=repmat([0 1],size(h.dat.graph.mimg,3),1);
...
ButtonDownFcn=@(hObject,eventdata)RoiGui_008('TimeSelectionButtonDownFcn',h.axes_fluorescence,eventdata,h);
...
h.dat.cl.shift_labelID = getOr(h.dat.cl,'shift_labelID',zeros(h.dat.ops.Nk, 1));
...
h.dat.cl.selected      = cat(1,h.dat.cl.selected,ones(dN,1));
    ...
    h.dat.cl.shift_labelID= cat(1,h.dat.cl.shift_labelID,zeros(dN,1));
...
switch eventdata.Key
    case {'w','r','g','b','p'}
            ...
            h.dat.graph.crange(h.dat.graph.map,:)=[h.sl_RGB_min.Value h.sl_RGB_max.Value];
...
%          fprintf('display selection=%d\n',h.dat.display_select);
    ...
    case 'r'
        ...
        h.dat.graph.map = 3;
        if size(h.dat.graph.mimg,3)>=3
            pb_Red_display_Callback(hObject, eventdata, h);
        end
        last_keypress = eventdata.Key;
    case 'g'
        
        h.dat.graph.map = 4;
        if size(h.dat.graph.mimg,3)>=4
            pb_Green_display_Callback(hObject, eventdata, h);
        end
        last_keypress = eventdata.Key;
    case 'b'
        
        h.dat.graph.map = 5;
        if size(h.dat.graph.mimg,3)>2
            pb_Blue_display_Callback(hObject, eventdata, h);
        end
        last_keypress = eventdata.Key;
    case 'p'
...
%             pos=[0.5 0.5 0.03 0.03];
            CallbackFcn=@(hObject,eventdata)RoiGui_008('enter_svm_value', hObject,eventdata,h);            
...
%             h.dat.F.ichosen_append ;
    ...
    switch SelectionType
        ...
        case 'open' % double click
            ...
            h = buildLambdaValue(h);
        case 'extend' % shift left
            UserData=h.bg_ShiftLabel.SelectedObject.UserData;
            LabelID=UserData{1};
            
            if (h.dat.cl.shift_labelID(h.dat.F.ichosen)==0)
                h.dat.cl.shift_labelID(h.dat.F.ichosen)=LabelID;
            else
                h.dat.cl.shift_labelID(h.dat.F.ichosen)=0;
            end
...
%         h=redraw_corrimg(h);
    case 'tb_SVMMode'
%         unique(h.dat.cl.svm_class);
        PlotHue = h.dat.cl.rands(h.dat.F.ichosen_append);
        PlotColor =permute(cat(2,PlotHue(:),ones(length(PlotHue),2)),[1,3,2]);
        PlotColor = hsv2rgb(PlotColor);
        
%         
%         hspace.unknown=[1 1 1]; % white
%         hspace.soma=[0 0 1]; % red
%         hspace.dendrite=[0 1 0]; % green;
%         hspace.axon    =[1 0 0]; % blue;
%         hspace.spine   =[0 1 1]; % yellow;
%         hspace.noise=[0 0 0]; % red
%         PlotHue=[hspace.unknown;...  %class label 0
%                  hspace.soma;...    %1
%                  hspace.dendrite;...%2
%                  hspace.axon;...    %3
%                  hspace.spine;...   %4
...
%                  hspace.noise]; %9
%              
%         Plot = PlotHue(h.dat.cl.svm_class+1,:);
%         PlotHue = 
...
%         ROIColor=h.popup_ROI_ColorSelection.String{h.popup_ROI_ColorSelection.Value};
        ROIColor='Green'; % alway use green     
        switch ROIColor
            case 'Green'
                PlotHue =0.333;
            case 'Red'
...
% 
function h=set_sl_RGB_maxmin(h)

h.sl_RGB_min.Value=h.dat.graph.crange(h.dat.graph.map,1);
h.sl_RGB_max.Value=h.dat.graph.crange(h.dat.graph.map,2);
...
if h.dat.graph.map<=2 % correlation map
    
    set(h.axes_left,'XLim',[h.dat.QV.xlim],'YLim',[h.dat.QV.ylim],'CLim',mu + [-2.5*sd1 4*sd2]);
else % RGB 3,4,5
    maxI = max(I(:));

    crange=[mu*h.sl_RGB_min.Value h.sl_RGB_max.Value*maxI];
    set(h.axes_left,'XLim',[h.dat.QV.xlim],'YLim',[h.dat.QV.ylim],'CLim',crange);
end
...
switch Tag
    ...
    case 'tb_SVMMode'
       ...
       for ii=1:h.dat.ops.Nk
            ...
            if isnan(edge_pix) continue; end
    ...
    case 'tb_LabelMode'
        ...
        Label2Col=[0.333, 0, 0.667,0.16]; % green, red, blue, yellow
...
%          ROIColor=h.popup_ROI_ColorSelection.String{h.popup_ROI_ColorSelection.Value};
        ...
        PlotHue=Label2Col(h.dat.cl.shift_labelID+1);
...
if isfield(h.dat.res,'probabilities')
    ...
    h.dat.res.probabilities=ones(h.dat.res.Ly,h.dat.res.Lx);
end
...
% if any(size(h.dat.res.M0)==1), h.dat.res.M0=reshape(h.dat.res.M0,[])
    if any(size(h.dat.res.M0)==1), h.dat.res.M0=reshape(h.dat.res.M0,[h.dat.res.Ly,h.dat.res.Lx]); end
...
% from RoiGui_008, use mlttiff class to access multiple Tiff files as a single object. 
...
switch Tag
    case 'tg_DisplayMode'
        ...
        ButtonDownFcn=@(hObject,eventdata)RoiGui_008('CellSelectionButtonDownFcn',...
            hObject,eventdata,guidata(hObject));
        set(handles.left_imageH,'ButtonDownFcn',ButtonDownFcn);
         set(handles.uipanel_Left,'Title','DisplayMode');
    case 'tg_EllipseMode'
        
        ButtonDownFcn=@(hObject,eventdata)RoiGui_008('EllipseSelectionButtonDownFcn',...
            hObject,eventdata,guidata(hObject));
        set(handles.left_imageH,'ButtonDownFcn',ButtonDownFcn);
...
%     case 'tg_ROIMergeMode' % merge is not considered as a mode. Merge already selected rois. 
    ...
    case 'tg_ROISplitMode'
        ...
        ButtonDownFcn=@(hObject,eventdata)RoiGui_008('CellSplitButtonDownFcn',...
            hObject,eventdata,guidata(hObject));
        set(handles.left_imageH,'ButtonDownFcn',ButtonDownFcn);
    ...
    case 'tb_SVMMode'
        ...
        ButtonDownFcn=@(hObject,eventdata)RoiGui_008('CellSelectionButtonDownFcn',...
            hObject,eventdata,guidata(hObject));
%         ButtonDownFcn=@(hObject,eventdata)RoiGui_008('SVMSelectionButtonDownFcn',...
%             hObject,eventdata,guidata(hObject));
        set(handles.left_imageH,'ButtonDownFcn',ButtonDownFcn);
...
if (do_padding_inf)
    ...
    for ii=1:size(data,2)
        tmp=data(:,ii);
        infind=isinf(tmp);
        tmpwoinf=tmp; tmpwoinf(infind)=[];
        tmp(infind)=nanmean(tmpwoinf);
        data(:,ii)=tmp;
    end
end

data = nanzscore(data,0,1);
...
% --- Executes on button press in checkbox_Label1_Shiftclick.
function checkbox_Label1_Shiftclick_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_Label1_Shiftclick (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox_Label1_Shiftclick


% --- Executes on button press in checkbox_Label2_Shiftclick.
function checkbox_Label2_Shiftclick_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_Label2_Shiftclick (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox_Label2_Shiftclick
...
RGBImg= Load_register_OtherChannels_20181001(h);
h.dat.graph.RGBImg=RGBImg;
h=set_RGB_img_to_ming_mingproc(h,RGBImg);
% h.dat.graph.mimg_proc(:,:,3)=RGBImg.R;
% h.dat.graph.mimg_proc(:,:,4)=RGBImg.G;
% h.dat.graph.mimg_proc(:,:,5)=RGBImg.B;

guidata(hObject,h);

function h=set_RGB_img_to_ming_mingproc(h,RGBImg)

h.dat.graph.mimg_proc(:,:,3)=RGBImg.R;
h.dat.graph.mimg_proc(:,:,4)=RGBImg.G;
h.dat.graph.mimg_proc(:,:,5)=RGBImg.B;

h.dat.graph.mimg(:,:,3)=RGBImg.R;
h.dat.graph.mimg(:,:,4)=RGBImg.G;
h.dat.graph.mimg(:,:,5)=RGBImg.B;

h.dat.graph.crange(3:5,:)=repmat([0 1],3,1);
...
% --- Executes on button press in pb_Red_display.
function pb_Red_display_Callback(hObject, eventdata, h)
% hObject    handle to pb_Red_display (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

h.dat.graph.map = 3;
h=set_sl_RGB_maxmin(h);

h=redraw_meanimg(h);
guidata(hObject,h);

% --- Executes on button press in pb_Green_display.
function pb_Green_display_Callback(hObject, eventdata, h)
% hObject    handle to pb_Green_display (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in pb_Blue_display.
function pb_Blue_display_Callback(hObject, eventdata, h)
% hObject    handle to pb_Blue_display (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on slider movement.
function sl_RGB_max_Callback(hObject, eventdata, h)
% hObject    handle to sl_RGB_max (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
h.dat.graph.crange(h.dat.graph.map,2)=hObject.Value;
h=redraw_meanimg(h);
guidata(hObject,h);

% --- Executes during object creation, after setting all properties.
function sl_RGB_max_CreateFcn(hObject, eventdata, handles)
% hObject    handle to sl_RGB_max (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on slider movement.
function sl_RGB_min_Callback(hObject, eventdata, h)
% hObject    handle to sl_RGB_min (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'Value') returns position of slider
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
h.dat.graph.crange(h.dat.graph.map,1)=hObject.Value;
h=redraw_meanimg(h);
guidata(hObject,h);

% --- Executes during object creation, after setting all properties.
function sl_RGB_min_CreateFcn(hObject, eventdata, handles)
% hObject    handle to sl_RGB_min (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function edit_Label1TypeName_Callback(hObject, eventdata, handles)
% hObject    handle to edit_Label1TypeName (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_Label1TypeName as text
%        str2double(get(hObject,'String')) returns contents of edit_Label1TypeName as a double


% --- Executes during object creation, after setting all properties.
function edit_Label1TypeName_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_Label1TypeName (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function edit_Label2TypeName_Callback(hObject, eventdata, handles)
% hObject    handle to edit_Label2TypeName (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_Label2TypeName as text
%        str2double(get(hObject,'String')) returns contents of edit_Label2TypeName as a double


% --- Executes during object creation, after setting all properties.
function edit_Label2TypeName_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_Label2TypeName (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes when selected object is changed in bg_ShiftLabel.
function bg_ShiftLabel_SelectionChangedFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in bg_ShiftLabel 
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in tg_LabelRed.
function tg_LabelRed_Callback(hObject, eventdata, handles)
% hObject    handle to tg_LabelRed (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on button press in tg_LabelBlue.
function tg_LabelBlue_Callback(hObject, eventdata, handles)
% hObject    handle to tg_LabelBlue (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on button press in tg_LabelYellow.
function tg_LabelYellow_Callback(hObject, eventdata, handles)
% hObject    handle to tg_LabelYellow (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
